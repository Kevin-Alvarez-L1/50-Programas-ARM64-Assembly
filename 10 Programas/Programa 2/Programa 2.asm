...
_start:
    ...
    add x21, x19, x20               // x21 = x19 (num1) + x20 (num2)

    // Mostrar el mensaje del resultado
    ldr x0, =1                      // STDOUT (descriptor de archivo 1)
    ldr x1, =resultMsg              // Cargar la dirección del mensaje
    mov x2, #12                     // Longitud del mensaje (12 caracteres)
    mov x8, #64                     // Syscall para 'write' (64)
    svc #0                          // Llamada al sistema

    // Convertir el resultado de la suma a cadena y mostrarlo
    mov x0, x21                     // Mover el resultado (suma) a x0
...
// Función para mostrar el resultado (convertir número a str